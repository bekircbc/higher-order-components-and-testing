...
export const convertBackSlashesToForwardSlashes = (pathAndFileName: string) =>
...
export const linkify = (inputText: string) => {
  ...
  let replacePattern3 =
    /(([a-zA-Z0-9\-\_\.])+@[a-zA-Z\_]+?(\.[a-zA-Z]{2,6})+)/gim; // eslint-disable-line
  replacedText = replacedText.replace(
    replacePattern3,
    '<a href="mailto:$1">$1</a>'
  );
 ...
 */
export const replaceAllRegex = function (
  text: string,
  regex: RegExp,
  replace: string
) {
...
export const getSingularPluralSyntaxVariations = (
  ...
  syntaxVariations[`${prefix}PascalPlural`] =
    qstr.forcePascalNotation(camelPlural);
  syntaxVariations[`${prefix}PascalSingular`] =
    qstr.forcePascalNotation(camelSingular);
  syntaxVariations[`${prefix}TitlePlural`] =
    qstr.forceTitleNotation(camelPlural);
  syntaxVariations[`${prefix}TitleSingular`] =
    qstr.forceTitleNotation(camelSingular);
  syntaxVariations[`${prefix}TextPlural`] = qstr.forceTextNotation(camelPlural);
  syntaxVariations[`${prefix}TextSingular`] =
    qstr.forceTextNotation(camelSingular);
...
export const getTermSyntaxVariations = (
  termTitleNotation: string,
  prefix: string
) => {
  const syntaxVariations: any = {};
  syntaxVariations[`${prefix}Title`] = termTitleNotation;
  syntaxVariations[`${prefix}Text`] = qstr.forceTextNotation(termTitleNotation);
  syntaxVariations[`${prefix}Camel`] =
    qstr.forceCamelNotation(termTitleNotation);
  syntaxVariations[`${prefix}Pascal`] =
    qstr.forcePascalNotation(termTitleNotation);
...
export const removeEndMarkerAndGetNumberOfPrecedingTabsAndLine = (
  line: string,
  marker: string
): any[] => {
